...
const AdminFullProjects = () => {
  ...
  const [activeItem, setActiveItem] = useState<FullProjectResponse | null>(null);
  ...
  const form = useForm<FullProjectFormValues>({
    resolver: zodResolver(fullProjectSchema),
    defaultValues: { title: "", description: "", externalLink: "" },
  });

  const createMutation = useMutation({
    mutationFn: (payload: FullProjectPayload) => createFullProject(token, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["full-projects"] });
      toast({ title: "Full project created" });
      handleCloseDialog();
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["full-projects"] });
      toast({ title: "Full project updated" });
      handleCloseDialog();
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["full-projects"] });
      toast({ title: "Full project deleted" });
    },
  });
  ...
  const openEditDialog = (item: FullProjectResponse) => {
    setDialogMode("edit");
    setActiveItem(item);
    form.reset({
      title: item.title,
  ...
  const handleSubmit = form.handleSubmit(async (values) => {
    ...
    if (dialogMode === "create") {
      ...
      const updatePayload: Partial<FullProjectPayload> = {
        title: values.title.trim(),
        description: values.description?.trim() || undefined,
        externalLink: values.externalLink.trim(),
      };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Full Projects</h1>
          <p className="text-sm text-slate-400">Manage your complete campaign projects</p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center text-slate-400">
            No full projects yet. Add your first project!
          </CardContent>
        </Card>
      ) : (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{dialogMode === "create" ? "Add Full Project" : "Edit Full Project"}</DialogTitle>
            <DialogDescription>
              {dialogMode === "create" ? "Create a new complete campaign project" : "Update project details"}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
...
export default AdminFullProjects;
